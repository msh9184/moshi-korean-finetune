...
def import_model(
    ...
    for k, v in sorted(tch_model.items()):
        if k.startswith("transformer"):
            # Rust server expects specific naming convention:
            # - in_proj_weight (underscore, no dot before weight)
            # - out_proj.weight (dot before weight)
            # See import_rust.py line 91-92 comment for reference
            k = k.replace(".in_projs.0.weight", ".in_proj_weight")
            k = k.replace(".out_projs.0.weight", ".out_proj.weight")
            # Also handle if model already uses singular form
            if ".self_attn.in_proj.weight" in k:
                k = k.replace(".self_attn.in_proj.weight", ".self_attn.in_proj_weight")
            model[k] = v
    ...
    # Print first transformer layer tensors for verification
    print("\n=== Output Tensor Names (transformer layer 0) ===")
    for k in sorted(model.keys()):
        if "transformer.layers.0." in k:
            print(f"  {k}")
    print("=" * 50)
...
def main():
    ...
    # When loading LoRA weights, we need to enable LoRA mode in the model
    # lora_rank and lora_scaling should match the training configuration
    lora_overrides = {}
    if args.lora_weight is not None:
        lora_overrides = {
            'lora': True,
            'lora_rank': 32,      # Match training config
            'lora_scaling': 2.0,  # Match training config
        }
    ...
    lm = checkpoint_info.get_moshi(
        ...
        fuse_lora=True,
        lm_kwargs_overrides=lora_overrides
    )
