...
```mermaid
flowchart TD
    subgraph ORIG["Original Moshi Stream [9, T_full]"]
        OT["Text:  [안녕][하세요][PAD][오늘][날씨][좋네요][PAD][네][감사]"]
        OA["Audio: [C0-7] x9 frames ..."]
    end
    ORIG -->|"Random Sampling (10-15초)"| REF
    subgraph REF["Reference Prompt (exclude_start~exclude_end 회피)"]
        RT["Text:  [안녕][하세요]  (Text tokens, Moshi text stream)"]
        RA["Audio: [C0-7][C0-7]  (Audio codes, 8 Mimi codebooks)"]
    end
    REF -->|"Prepend to Main Sequence"| SEQ
    subgraph SEQ["Prompted Sequence"]
        P["PROMPT (Loss 제외)<br/>Text:  [안녕][하세요]<br/>Audio: [C0-7][C0-7]<br/>prompt_mask=True"]
        M["MAIN (Loss 계산)<br/>Text:  [오늘][날씨][좋네요][PAD][네][감사]<br/>Audio: [C0-7] x6 frames<br/>prompt_mask=False"]
        P --- M
    end
```
...
```mermaid
flowchart TD
    RAW["Reference Audio (10-15초)<br/>[Raw Audio Waveform]"]
    RAW --> ENC["Speaker Encoder<br/>(W2v-BERT 2.0 SV)<br/>Output: [B, 256]<br/>(Global Embedding)"]
    RAW --> SAMP["Audio Prompt Sampler<br/>(Mimi Codes + Text Tokens)<br/>Output: [B, 9, T_prompt]<br/>(Audio[8] + Text[1])"]
    ENC --> COND["Speaker Conditioner<br/>Linear(256 to 4096)<br/>Output: sum_condition"]
    SAMP --> PRE["Prepend to Sequence<br/>[Prompt | Main Sequence]<br/>Output: prompted_codes"]
    COND --> TT["Temporal Transformer<br/>hidden_states = hidden_states + sum_condition  (Global Condition)<br/>attention([Prompt | Main])  (Local Condition)"]
    PRE --> TT
```
