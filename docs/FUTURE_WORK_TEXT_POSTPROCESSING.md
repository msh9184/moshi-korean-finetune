...
```mermaid
flowchart TD
  T["character_level_interpolation=True 설정 시"]
  A["입력 alignments:<br/>(안녕, (0.0, 0.5), SPEAKER_MAIN)<br/>(하세요, (0.6, 1.0), SPEAKER_MAIN)"]
  B["_word_to_character_alignments():<br/>[(안,0.0,0.25), (녕,0.25,0.5), (하,0.6,0.73), ...]"]
  C["_tokenize_with_character_interpolation():<br/>full_text = .join(char for char, _, _ in char_alignments)<br/>결과: 안녕하세요 ← 단어 사이 공백 없음!"]
  D["tokenize():<br/>SentencePiece가 공백 없는 텍스트를 토큰화<br/>→ ▁ 마커가 단어 경계에서 생성되지 않음"]
  E["모델 학습:<br/>모델이 띄어쓰기 없는 텍스트를 정답으로 학습<br/>→ 추론 시에도 띄어쓰기 없이 출력"]
  T --> A
  A --> B
  B --> C
  C --> D
...
flowchart TD
  IN["모델 출력 텍스트"]
  S1["Stage 1: 노이즈 정규화 (Noise Normalization)<br/>• SentencePiece 마커 제거: ▁ → 공백<br/>• 특수문자 정규화: 백슬래시, /, _ 제거<br/>• 반복 문자 정리: ㅋㅋㅋㅋㅋ → ㅋㅋ<br/>• 인코딩 깨짐 복구: 깨진문자 → 제거<br/>• 지연시간: 1ms 미만<br/>• 구현: 정규표현식 기반 (모든 언어 공통)"]
  S2["Stage 2: 띄어쓰기/토큰화 교정 (Spacing/Tokenization)<br/>• 한국어: 형태소 분석 기반 띄어쓰기 복원<br/>• 영어: 단어 경계 정규화 (이미 양호한 경우 스킵)<br/>• 지연시간: 5-15ms<br/>• 도구: Lindera (Rust) - 한국어/일본어/중국어 지원"]
  S3["Stage 3: 구두점 복원 (Punctuation Restoration)<br/>• 문장 끝 마침표/물음표/느낌표 추가<br/>• 의문문 패턴 감지: ~까, ~니, ~죠 → ?<br/>• 지연시간: 1ms 미만<br/>• 구현: 규칙 기반 (언어별 패턴)"]
  S4["Stage 4: ITN - Inverse Text Normalization (선택적)<br/>• 숫자 변환: 천구백구십일 → 1991<br/>• 약어 변환: 에이아이 → AI, 지피티 → GPT<br/>• 단위 변환: 퍼센트 → percent, 달러 → dollar<br/>• 지연시간: 1-3ms<br/>• 구현: 규칙 기반 매핑 테이블 (언어별)<br/>• 참고: NVIDIA NeMo ITN은 한국어 미지원 (2025.01 기준)"]
  OUT["최종 출력 텍스트<br/>총 예상 지연시간: 7-20ms (Moshi 프레임 간격 80ms 대비 허용 가능)"]
  IN --> S1
  S1 --> S2
  S2 --> S3
  S3 --> S4
...
```mermaid
flowchart TD
  subgraph TRAIN["학습 파이프라인 - Python"]
    A1["train.py"]
    A2["sample_saver.py"]
    A3["KoreanTextPostProcessor (선택적)<br/>• 학습 로그 가독성 향상용<br/>• 설정: text_postprocessing.enabled"]
    A1 --> A2
    A2 --> A3
  end
  subgraph SERVE["서빙 파이프라인 - Rust"]
    B1["moshi-backend/"]
    B2["stream_both.rs"]
    B3["text() 메소드"]
    B4["TextPostProcessor::process() (선택적)<br/>• 런타임 토글 가능<br/>• 설정: config.text_postprocessing.enabled"]
    B1 --> B2
    B2 --> B3
    B3 --> B4
  end
  subgraph CLIENT["클라이언트 - JavaScript"]
    C1["Web Client"]
    C2["백업 후처리 (Rust에서 처리 안 된 경우)<br/>• 최소한의 정규화만 수행"]
    C1 --> C2
  end
```
