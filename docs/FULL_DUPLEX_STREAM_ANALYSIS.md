...
```mermaid
flowchart TD
    subgraph IN["INPUT STREAMS (17 codebooks) - CORRECT FULL-DUPLEX ARCHITECTURE (Original Moshi / J-Moshi 방식)"]
        TEXT["Text (Inner Monologue)<br/>idx=0"]
        MA0["Moshi Audio Codebook 0<br/>idx=1"]
        MA1["Moshi Audio Codebook 1<br/>idx=2"]
        MDOTS["..."]
        MA7["Moshi Audio Codebook 7<br/>idx=8"]
        UA0["User Audio Codebook 0<br/>idx=9"]
        UA1["User Audio Codebook 1<br/>idx=10"]
        UDOTS["..."]
        UA7["User Audio Codebook 7<br/>idx=16"]
    end

    TEXT -->|"text_emb(codes[:,0])"| COMBINED
    MA0 -->|"audio_emb[0](codes[:,1])"| SUM["Σ"]
    MA1 -->|"audio_emb[1](codes[:,2])"| SUM
    MDOTS -->|"..."| SUM
    MA7 -->|"audio_emb[7](codes[:,8])"| SUM
    UA0 -->|"audio_emb[8](codes[:,9])"| SUM
    UA1 -->|"audio_emb[9](codes[:,10])"| SUM
    UDOTS -->|"..."| SUM
    UA7 -->|"audio_emb[15](codes[:,16])"| SUM

    SUM --> COMBINED["combined_input = text_emb + Σ(audio_emb[0:16])<br/>16개 audio embedding 모두 합산 (Moshi 8개 + User 8개)"]
    COMBINED --> TEMP["Temporal Transformer (Backbone)<br/>User context 포함!"]
    TEMP --> DEPTH["Depth Transformer (Depformer)"]
    DEPTH -->|"Moshi Audio 8개 예측 (dep_q=8)"| OUT["Output"]
```
...
```mermaid
flowchart TD
    subgraph IN["INPUT STREAMS (17 codebooks) - CURRENT K-MOSHI IMPLEMENTATION (BUG: User Audio Not Processed!)"]
        TEXT["Text (Inner Monologue)<br/>idx=0"]
        MA0["Moshi Audio Codebook 0<br/>idx=1"]
        MA1["Moshi Audio Codebook 1<br/>idx=2"]
        MDOTS["..."]
        ...
        UDOTS["..."]
        UA7["User Audio Codebook 7<br/>idx=16"]
        IGNORED["audio_embs has only 8 elements!<br/>No audio_emb[8] to audio_emb[15]<br/>User audio streams are COMPLETELY IGNORED!"]
    end
    ...
    MDOTS -->|"..."| SUM
    MA7 -->|"audio_emb[7](codes[:,8])"| SUM
    UA0 -->|"NO EMBEDDING!"| IGNORED
    UA1 -->|"NO EMBEDDING!"| IGNORED
    UDOTS -->|"..."| IGNORED
    UA7 -->|"NO EMBEDDING!"| IGNORED

    SUM --> COMBINED["combined_input = text_emb + Σ(audio_emb[0:8]) (ONLY 8!)<br/>Moshi 8개만 합산, User 8개 누락!"]
    COMBINED --> TEMP["Temporal Transformer (Backbone)<br/>User context 없음! 대화 맥락 손실!"]
    TEMP --> DEPTH["Depth Transformer (Depformer)"]
    DEPTH -->|"Moshi Audio 8개 예측 (dep_q=8)"| OUT["Output"]
```
